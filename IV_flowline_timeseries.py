...
Inputs:
    ...
    bed_file:       string with directory+name of bedmachine file for use in inset map

Creates a .png of flowline velocity plot.

Beware: Due to different operators with varying QGis experience at different times, the shape files 
are sometimes problematic to work with, and some bad practice coding is implemented with try-except below.
...
def IV_flowline_timeseries(glacier, IV_file, CFL_file, FL_file, cm_file, outfile, mean_IV_file, bed_file):
    ...
    v_int = RectBivariateSpline(X_v, Y_v[::-1], vel[::-1, :].T, kx=1, ky=1, s=0)         # More efficient alternative to interp2d. Note1: Requires strictly increasing coordinate vectors, thus [::-1] in both y-vector and v-matrix. Note2: Takes in z-values transposed relative interp1d, thus .T
    ...
    mean_v_int = RectBivariateSpline(X_v, Y_v[::-1], mean_v[::-1, :].T, kx=1, ky=1)
    mean_v_fl = mean_v_int(X_fl, Y_fl, grid=False)
    mean_v_fl[mean_v_fl > 2e4] = np.nan

    # Lower quantile stuff
    mean_v_lq = mean_IV[0, :, :]
    mean_v_lq[np.isnan(mean_v_lq)] = 1e20
    mean_v_lq_int = RectBivariateSpline(X_v, Y_v[::-1], mean_v_lq[::-1, :].T, kx=1, ky=1)
    mean_v_lq_fl = mean_v_lq_int(X_fl, Y_fl, grid=False)
    mean_v_lq_fl[mean_v_fl > 2e4] = np.nan

    # Upper quantile stuff
    mean_v_uq = mean_IV[2, :, :]
    mean_v_uq[np.isnan(mean_v_uq)] = 1e20
    mean_v_uq_int = RectBivariateSpline(X_v, Y_v[::-1], mean_v_uq[::-1, :].T, kx=1, ky=1)
    mean_v_uq_fl = mean_v_uq_int(X_fl, Y_fl, grid=False)
    mean_v_uq_fl[mean_v_fl > 2e4] = np.nan
    ...
    ax1.plot(dist_fl, mean_v_fl, color='black', alpha=0.5, label='Median velocity for ' + smb_start_str + ' - ' + smb_end_str)
    ax1.fill_between(dist_fl, mean_v_lq_fl, mean_v_uq_fl, alpha=0.1, color='black', label='Lower and upper quartiles')
    ...
    uncertainty_mask = v_std > 1000 * vel # Set uncertain values to nan
